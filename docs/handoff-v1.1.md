...
- 当前阶段：strict 阶段。`npm run audit:concepts` 默认阻断 codeExamples 缺口；`npm run audit:concepts -- --strict-code-examples` 仅作为旧发布命令兼容。
...
- codeExamples strict 化已完成；剩余发布动作是跑全量 gates，推 `v1.1-rc` PR，绿灯后合并并打 `v1.1.0` annotated tag。
...
1. 跑 `npm run audit:concepts` 和 `npm run audit:concepts -- --strict-code-examples`，必须都返回 0。
2. 跑 `npm run test:code-examples`，必须返回 0。
3. `src/content.config.ts` 已把 `codeExamples` strict 化，并保留 `language` category 豁免。
4. `scripts/validate-relations.ts` 已把 codeExamples 缺失升级为 blocking error，并保持 `language` 豁免。
5. `docs/content-guidelines.md` 已删除 warning 阶段表述，保留复核标准。
6. `docs/deployment.md` 已把 codeExamples 加入严格字段清单。
...
- [x] `audit:concepts` 已默认阻断 codeExamples 缺口。
- [x] `audit:concepts -- --strict-code-examples` 保持兼容并返回 clean。
...
- [x] `src/content.config.ts` strict 化完成。
- [x] `scripts/validate-relations.ts` strict 化完成。
...
当前预期：默认 `audit:concepts` 已因 codeExamples 缺口阻断；`npm run audit:concepts` 和 `npm run audit:concepts -- --strict-code-examples` 都应返回 0。Week 9 strict 化事实已写回 schema、`validate-relations` 和部署文档。
