...
v1.0 已移除 `expandsTo`、`works.note` 和 concept 内联 `works[]`。新增内容只使用 `extends` 和 `worksRef[].role`；如果在旧内容或脚本里看到旧字段，应视为迁移遗漏并修正。
...
works registry 已落地：`src/content/works-registry.yaml` 保存作品稳定元数据，concept 使用 `worksRef[]` 引用。内联 `works[]` 不再保留兼容回退；再次出现时应由 `audit:concepts` 和 `validate:relations` 阻断。概念侧 `role` 仍必须说明作品如何体现当前概念。
